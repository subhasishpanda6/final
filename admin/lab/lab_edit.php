...
<body>
  ...
  if (isset($_POST['submit'])) {    
    $sql = $conn->prepare("UPDATE lab_registration SET  lab_name=? , email=?,password=?,address=?,city=?,pincode=?,joining_date=?  WHERE id=?");
    ...
    $sql->bind_param("sssssssi",$lab_name, $email,$password,$address,$city,$pincode,$joining_date,$_GET["id"]);  
    if($sql->execute()) {
  ...
  $sql = $conn->prepare("SELECT * FROM lab_registration WHERE id=?");
